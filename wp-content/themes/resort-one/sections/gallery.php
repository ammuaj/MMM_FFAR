...
$gallery_subtitle = get_theme_mod( 'gallery_subtitle', esc_html__( 'OUR COLLECTION', 'resort-one' ) );
$gallery_title    = get_theme_mod( 'gallery_title', esc_html__( 'Our Featured Gallery', 'resort-one' ) );
...
if ( $ed_section && ( $gallery_subtitle || $gallery_title || $gallery_content || $gallery_one || $gallery_two || $gallery_three || $gallery_four || $gallery_five ) ) { ?>
	<section class="gallery-archive gallery-main section-padding" id="gallery-section">
		<div class="container">
			<?php
			hotell_section_header( $gallery_title, $gallery_subtitle, $gallery_content );
				...
				if ( $gallery_qry->have_posts() ) {
					...
					<div class="gallery">
						...
						while ( $gallery_qry->have_posts() ) {
							...
							if ( 0 === $gallery_qry->current_post ) {
								$image_size = 'hotell-gallery-full';
							...
							if ( 0 === $gallery_qry->current_post ) {
								echo '<div class="large-small-post">';
							}
							if ( 0 === $gallery_qry->current_post || 1 === $gallery_qry->current_post || 3 === $gallery_qry->current_post ) {
							...
							<?php
							if ( 0 === $gallery_qry->current_post || 2 === $gallery_qry->current_post || 4 === $gallery_qry->current_post ) {
								echo '</div>';
							}
							if ( 2 === $gallery_qry->current_post ) {
								echo '</div>';
							}
						}
						wp_reset_postdata();
